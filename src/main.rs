fn get_topmost_int(stack: &mut [u8], stack_size: &mut usize) -> u32 {
    ...
    *stack_size = base_stack;
    ...
    (stack[base_stack] as u32) * 0x1000000
        + (stack[base_stack + 1] as u32) * 0x10000
        + (stack[base_stack + 2] as u32) * 0x100
        + (stack[base_stack + 3] as u32)
}
...
{
    let result = op(
        get_topmost_int(stack, stack_size),
        get_topmost_int(stack, stack_size),
    );
